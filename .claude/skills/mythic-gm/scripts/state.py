...
Commands:
  ...
  thread  add|weight|remove|note|show <campaign> ["name"] ["note"]   Manage threads.json
  char    add|weight|remove|note|show <campaign> ["name"] ["note"]   Manage characters.json
  adventure show <campaign>                            Show theme order / tens / scene / style
  ...
  render <campaign>                Regenerate the LISTS.md snapshot from the JSON (the snapshot
                                   is GENERATED, never hand-edited; auto-runs after every mutation)
...
def render_lists(campaign):
    """Generate campaign/LISTS.md from the JSON. The snapshot is GENERATED, never hand-edited —
    this is what kills dual-source drift: JSON is the sole source, LISTS.md is a view of it."""
    adv = lists.load_adventure(campaign)
    out = ["# Lists — GENERATED SNAPSHOT (do not hand-edit)",
           "",
           ...
           ""]
    for kind, title in (("thread", "Threads"), ("character", "Characters")):
        obj = lists.load_list(campaign, kind)
        S = lists.total_weight(obj["entries"])
        flag = "  ⚠ OVERFULL (>25 — full list still rolls over)" if S > 25 else ""
        out.append(f"## {title} — Σ {S} weighted slot(s){flag}")
        if not obj["entries"]:
            out.append("_(empty)_")
        for i, e in enumerate(obj["entries"], 1):
            w = int(e.get("weight", 1))
            line = f"{i}. {e['name']}" + (f"  ×{w}" if w > 1 else "")
            if e.get("note"):
                line += f" — {e['note']}"
            out.append(line)
        out.append("")
    out.append("## Adventure")
    out.append(f"- Theme priority: {', '.join(adv['theme_order'])}")
    out.append(f"- Tens-cycle: {adv['tens']}  ·  Scene #: {adv.get('scene', 0)}  ·  Style: {adv['style']}")
    out.append("")
    path = os.path.join(campaign, "LISTS.md")
    open(path, "w", encoding="utf-8").write("\n".join(out))
    return path

def cmd_render(campaign):
    print(f"Rendered {render_lists(campaign)} from JSON (threads/characters/adventure).")

def cmd_listcmd(kind, action, campaign, name=None, note=None):
    obj = lists.load_list(campaign, kind)
    if action == "show":
        _render(obj); return
    if action in ("add", "weight"):
        ...
        if note:
            e = lists._find(obj["entries"], name)
            if e is not None: e["note"] = note
        lists.save_list(campaign, obj); print(f"{kind} '{name}': {status}.")
    elif action == "note":
        e = lists._find(obj["entries"], name)
        if e is None:
            print(f"⚠ {kind} '{name}' not found — add it first."); return
        e["note"] = note or ""
        lists.save_list(campaign, obj); print(f"{kind} '{name}': note set.")
    elif action == "remove":
        status, obj = lists.remove_entry(obj, name)
        lists.save_list(campaign, obj); print(f"{kind} '{name}': {status}.")
    else:
        sys.exit(f"Unknown {kind} action '{action}' (add|weight|remove|note|show).")
    render_lists(campaign)   # keep the generated snapshot in lockstep with the JSON
    _render(obj)
...
def cmd_adventure(action, campaign, arg=None):
    ...
    if action == "show":
        ...
        print(f"Tens-cycle counter: {adv['tens']}   |   scene #: {adv.get('scene', 0)}   |   style: {adv['style']}")
...
def main():
    ...
    elif c == "thread": cmd_listcmd("thread", a[1], a[2], a[3] if len(a) > 3 else None, a[4] if len(a) > 4 else None)
    elif c == "char": cmd_listcmd("character", a[1], a[2], a[3] if len(a) > 3 else None, a[4] if len(a) > 4 else None)
